...
fn test_port_rules_map() {
    ...
    port_rules
        ...
        .expect("Failed to insert into PORT_RULES");

    // Verify we can read it back
    let retrieved = port_rules
        .get(&port, 0)
        .expect("Failed to get from PORT_RULES");
    assert_eq!(retrieved, action, "Port rule mismatch");

    // Test: Update to Pass
    let new_action = Action::Pass as u8;
    port_rules
        .insert(port, new_action, 0)
        .expect("Failed to update PORT_RULES");
    let retrieved = port_rules
        .get(&port, 0)
        .expect("Failed to get updated rule");
...
fn test_port_stats_map() {
    ...
    port_stats
        .insert(port, count, 0)
        .expect("Failed to insert into PORT_STATS");

    // Simulate increment (in real XDP program, this would happen on packet processing)
    let retrieved = port_stats
        .get(&port, 0)
        .expect("Failed to get from PORT_STATS");
    assert_eq!(retrieved, 0, "Initial stats should be 0");

    // Update stats
    port_stats
        .insert(port, 100, 0)
        .expect("Failed to update PORT_STATS");
    let retrieved = port_stats
        .get(&port, 0)
...
fn test_config_array() {
    ...
    config
        .set(CONFIG_LOG_LEVEL, log_level, 0)
        .expect("Failed to set log level");

    // Verify
    let retrieved = config
        .get(&CONFIG_LOG_LEVEL, 0)
    ...
    // Verify
    let retrieved = config
        .get(&CONFIG_IP_FILTER_MODE, 0)
        .expect("Failed to get filter mode");
...
fn test_ipv4_filter_map() {
    ...
    // Verify
    let retrieved = ip_filter
        .get(&ip, 0)
        .expect("Failed to get from IP_FILTER_LIST_V4");
...
fn test_ipv6_filter_map() {
    ...
    let retrieved = ip_filter
        .get(&ip, 0)
        .expect("Failed to get from IP_FILTER_LIST_V6");
...
fn test_rate_limit_state_v4_map() {
    ...
    // Verify
    let retrieved = rate_limit
        .get(&ip, 0)
        .expect("Failed to get from RATE_LIMIT_STATE_V4");
    ...
    assert_eq!(
        ...
        "Timestamp mismatch after unpack"
    );
    assert_eq!(
        retrieved_packet_count, packet_count,
        "Packet count mismatch after unpack"
    );
}
...
fn test_stats_array() {
    ...
    stats
        .set(STAT_TOTAL_PACKETS, 1000, 0)
        .expect("Failed to set total packets");
    stats
        .set(STAT_TCP_PACKETS, 600, 0)
        .expect("Failed to set TCP packets");
    stats
        .set(STAT_UDP_PACKETS, 400, 0)
        .expect("Failed to set UDP packets");
    stats
    ...
    // Verify all stats
    let total = stats
        .get(&STAT_TOTAL_PACKETS, 0)
        .expect("Failed to get total packets");
    let tcp = stats
        .get(&STAT_TCP_PACKETS, 0)
        .expect("Failed to get TCP packets");
    let udp = stats
        .get(&STAT_UDP_PACKETS, 0)
        .expect("Failed to get UDP packets");
    let substrate = stats
        .get(&STAT_SUBSTRATE_PACKETS, 0)
        .expect("Failed to get Substrate packets");

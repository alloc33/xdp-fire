    ...
    <T as TryFrom<Map>>::Error: std::error::Error + Send + Sync + 'static,
{
    ...
    let map_data = MapData::from_pin(&map_path).context(format!(
        "Failed to open pinned {} map. Is the XDP program running?",
        map_name
    ))?;
...
async fn handle_command(command: &Commands) -> anyhow::Result<()> {
    match command {
        Commands::SetLogLevel { level } => {
            ...
            let level_str = LogLevel::try_from(*level)
                .ok()
                .map(|l| l.as_str())
                .unwrap_or("UNKNOWN");
        ...
        Commands::SetIpFilterMode { mode } => {
            ...
            let mode_str = IpFilterMode::try_from(*mode)
                .ok()
                .map(|m| m.as_str())
                .unwrap_or("UNKNOWN");
        ...
        Commands::EnableRateLimit { pps, window_ms } => {
            ...
            info!(
                "✅ Enabled rate limiting: {} packets per {} ms",
                pps, window_ms
            );
        }
...
async fn main() -> anyhow::Result<()> {
    ...
    info!(
        "📍 Pinned IP_FILTER_LIST_V4 map to {}/IP_FILTER_LIST_V4",
        MAP_PIN_PATH
    );
    info!(
        "📍 Pinned IP_FILTER_LIST_V6 map to {}/IP_FILTER_LIST_V6",
        MAP_PIN_PATH
    );
    info!(
        "📍 Pinned RATE_LIMIT_STATE_V4 map to {}/RATE_LIMIT_STATE_V4",
        MAP_PIN_PATH
    );
    info!(
        "📍 Pinned RATE_LIMIT_STATE_V6 map to {}/RATE_LIMIT_STATE_V6",
        MAP_PIN_PATH
    );
